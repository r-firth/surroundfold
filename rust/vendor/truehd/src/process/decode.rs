// Modified for SurroundFold: correct presentation reselection and access-unit
// padding state, with regression coverage for both fixes.
...
impl DecoderState {
    ...
    pub fn decode_access_unit(
        ...
        if !self.valid || self.substream_info_changed {
...
mod tests {
    ...
    use crate::process::{EXAMPLE_DATA, PresentationMap, extract::Extractor, parse::Parser};

    #[test]
    fn access_unit_tail_padding_does_not_leak_into_the_next_unit() {
        ...
        for frame in extractor {
            ...
            access_units.push(parser.parse(&frame).unwrap());
        }
        assert!(access_units.len() >= 2);

        let mut decoder = Decoder::default();
        decoder.decode_presentation(&access_units[0], 0).unwrap();
        decoder.state.zero_samples = 7;
        let decoded = decoder.decode_presentation(&access_units[1], 0).unwrap();

        assert_eq!(decoded.sample_length, decoder.state.samples_per_au);
    }

    #[test]
    fn unavailable_presentation_is_reselected_after_a_map_change() {
        let mut state = DecoderState {
            valid: true,
            presentation: 3,
            presentation_map: Some(PresentationMap::with_substream_info(0b0101_1000, 0)),
            ..DecoderState::default()
        };

        state.update_presentation(3).unwrap();

        assert_eq!(state.presentation, 2);
    }
}
